...
public class AnnotationProxy extends KrollProxy
{
	...
	{
		if(isInteger(image)){	// image resource id
			int iconResId = 0;
			iconResId=(Integer)image;
			/*if(image instanceof String){
				try {
					Log.i(TAG, "iconResId: " + iconResId);
					iconResId=Integer.parseInt((String)image);
				} catch(Exception e) {
			...
			//setIconImageHeight(icon.getHeight());
			return;
		}else if (image instanceof String) {	// image path
			TiDrawableReference imageref = TiDrawableReference.fromUrl(this, (String) image);
			Bitmap icon = imageref.getBitmap();
			if (icon != null) {
